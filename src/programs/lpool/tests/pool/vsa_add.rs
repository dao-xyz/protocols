...
use {
    ...
    lpool::id,
    lpool::{error::StakePoolError, find_stake_program_address, instruction, state},
    solana_program::{
...
async fn setup() -> (
    ...
    let validator_stake = ValidatorStakeAccount::new(&stake_pool_accounts.stake_pool, 0);
    create_vote(
        &mut banks_client,
        &payer,
        &recent_blockhash,
...
async fn fail_on_non_vote_account() {
    ...
    let (stake_account, _) =
        find_stake_program_address(&id(), &validator, &stake_pool_accounts.stake_pool);
