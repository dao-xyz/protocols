...
use lpost::{
    ...
    instruction::{cast_vote, create_rule, deposit_governing_tokens},
    state::{
        post::{deserialize_post_account, PostAccount},
        rules::rule::{AcceptenceCriteria, MintWeight, RuleTimeConfig, RuleVoteConfig},
...
impl TestUser {
    ...
    pub fn get_token_owner_record_address(
        ...
        get_token_owner_record_address(&lpost::id(), mint, &self.payer)
    }
    ...
    pub async fn get_token_owner_record(
        ...
        let address = self.get_token_owner_record_address(banks_client, mint);
...
impl TestPost {
    pub async fn new(channel: &Pubkey) -> Self {
        ...
        let _upvote_mint = find_post_upvote_mint_program_address(&lpost::id(), &post).0;
        let _downvote_mint = find_post_downvote_mint_program_address(&lpost::id(), &post).0;
...
impl TestChannel {
    pub async fn new(
        ...
        let channel = create_and_verify_channel(
            ...
            channel_name.as_ref(),
            &creator_user.user,
            initial_governance_mint_address,
            None,
        )
        .await
        .unwrap();
