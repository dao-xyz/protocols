...
use crate::accounts::{deserialize_post_account, PostAccount, PostContentAccount};
use crate::{
...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
    ...
    match instruction {
        ...
        ChatInstruction::CreatePostContent(content) => {
            ...
            let rent = Rent::get()?;
            msg!(Pubkey::new_from_array(hash).to_string().as_str());
        ...
        ChatInstruction::StakePost(stake) => {
            ...
            let mint_account_seeds =
                create_escrow_account_bump_seeds(&stake.post, escrow_bump_seeds);
            let expected_escrow_address =
                Pubkey::create_program_address(&mint_account_seeds, program_id).unwrap();
            ...
            spl_mint_to(
                ...
                get_allowed_mint_amount(
                    escrow_account_info,
                    stake.stake,
                    post_account.stagnation_factor,
                ),
                program_id,
            )?;
        }
